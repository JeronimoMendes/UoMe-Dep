...
def debt_dashboard(request):
    ...
    if request.method == "GET":
        user = request.user
        common_accs = [(i, i.other_user(user), i.how_much_debt(user)) for i in CommonAccount.objects.filter(user1=user)]
        common_accs += [(i, i.other_user(user), i.how_much_debt(user)) for i in CommonAccount.objects.filter(user2=user)]
